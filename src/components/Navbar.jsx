        ...
        <div className="hidden lg:flex items-center gap-8">
          {navLinks.map((link) => (
            <a key={link.href} href={link.href}
              className="text-sm font-medium text-text-muted dark:text-slate-400 hover:text-primary dark:hover:text-blue-400 transition-colors">
              {link.label}
            </a>
          ))}
        </div>
          ...
          <div className="px-4 py-4 space-y-1">
            {navLinks.map((link) => (
              <a key={link.href} href={link.href}
                ...
                className="block px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-slate-100 dark:hover:bg-white/5 transition-colors">
                {link.label}
              </a>
            ))}
